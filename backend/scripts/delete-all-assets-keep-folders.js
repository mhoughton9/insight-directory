/**
 * Delete All Assets But Keep Folders Script
 * 
 * This script will delete ALL assets from Cloudinary,
...
async function deleteResourcesInBatches(resources) {
  try {
    ...
    console.log(`Found ${totalResources} assets to delete`);
    ...
    // Group resources by folder for better reporting
    const folderGroups = {};
    ...
    // Report assets by folder
    console.log('\nAssets to be deleted by folder:');
    Object.keys(folderGroups).sort().forEach(folder => {
      console.log(`${folder}: ${folderGroups[folder].length} assets`);
    });
    console.log('');
...
// Clear all Cloudinary assets but keep folders
const clearAllCloudinaryAssets = async () => {
  try {
    console.log('\n=== Clearing ALL Cloudinary Assets (Keeping Folders) ===');
...
const main = async () => {
  try {
    console.log('\n==================================');
    console.log('CLOUDINARY ASSETS CLEANUP');
    console.log('==================================');
    console.log('WARNING: This will delete ALL assets from Cloudinary,');
    console.log('but preserve the folder structure.');
    ...
    readline.question('Are you sure you want to proceed? (yes/no): ', async (answer) => {
      if (answer.toLowerCase() === 'yes') {
        console.log('\nProceeding with Cloudinary assets cleanup...');
        
        // Clear all Cloudinary assets
        const success = await clearAllCloudinaryAssets();
        
        if (success) {
          console.log('\n✅ Cloudinary assets cleanup completed successfully!');
        } else {

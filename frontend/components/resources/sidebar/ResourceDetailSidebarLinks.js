  ...
  const getLinkIcon = (url, label) => {
    try {
      ...
      if (hostname.includes('amazon') || hostname.includes('amzn.to')) {
        // Using a simplified book icon with stroke instead of fill
        return (
          <svg 
      ...
      if (hostname.includes('goodreads')) {
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-2 flex-shrink-0" fill="none" stroke="#553B08" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {links.map((link, index) => {
        ...
        const icon = getLinkIcon(url, label); // Pass label here

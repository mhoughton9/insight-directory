...
async function examineRetreatCenters() {
  try {
    ...
    for (const center of retreatCenters) {
      ...
      if (center.publishedDate) publishedDateCount++;
      if (center.dateRange && (center.dateRange.start || center.dateRange.end)) dateRangeCount++;
    ...
    console.log(`Retreat centers with publishedDate: ${publishedDateCount}`);
    console.log(`Retreat centers with dateRange: ${dateRangeCount}`);
    ...
    if (creatorCount !== retreatCenters.length) {
      console.log('- Need to standardize creator field for retreat centers');
    }
    
    if (publishedDateCount === 0 && dateRangeCount === 0) {
      console.log('- Consider adding date information to retreat centers');
    }
